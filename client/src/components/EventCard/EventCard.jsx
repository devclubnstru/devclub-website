...
const EventCard = (props) => {
    return (
        <article className="dark:bg-background-dark-base bg-background-light-base border border-black/20 dark:border-white/20 rounded-lg flex flex-col sm:p-4 p-2 w-full min-w-0 dark:shadow-none shadow-lg">
            {/* Top Meta */}
            <div className="mb-2">
                <h3 className="text-xs font-light dark:text-white/40 text-text-muted-light/50">{props.yearConducted}</h3>
                <h1 className="sm:text-lg text-sm font-semibold dark:text-white">{props.eventName}</h1>
            </div>
            ...
            <p className="text-xs text-center my-2 text-text-muted-light/70 dark:text-text-muted-dark/70">{props.eventType}</p>
